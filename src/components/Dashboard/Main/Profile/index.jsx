...
const Index = ({ profile }) => {
    ...
    const fetchEngineer = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/engineer/get?id=${engg?._id}`
        );
      ...
      try {
        const followers = await Promise.all(
          engineer?.followers?.map(async (f) => {
            const res = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL}/engineer/get?id=${f}`
            );
            return res.data;
          })
        );
  ...
  useEffect(() => {
    // This effect will run whenever `section` changes
    // It updates the URL with the new tab parameter
	console.log("section",section)
    router.push(`/profile?tab=${section}`);
  }, [section, router]);

  const handleButtonClick = (selectedSection) => {
    // Update the section state when a button is clicked
  ...
  return (
    ...
    >
      <Box height={"30vh"} weight={"100%"} position={"relative"}>
        <Image
      ...
      >
        ...
        >
          ...
          <Box display={"flex"} flexDirection={"column"} alignItems={"center"}>
            <Typography fontSize={25}>{profile.name}</Typography>
            <Typography fontSize={17} color={"#F5F5F5"}>
              {profile.email}
            </Typography>
          </Box>
          <Box
      ...
      >
        <Box>
          <Button
            sx={{
              borderBottom:
                section === "progress" ? "2px solid #05D9D7" : "0px",
              p: 0,
              px: "12px",
              borderRadius: "0px",
        ...
        <Box>
          <Button
            sx={{
              borderBottom:
                section === "questions" ? "2px solid #05D9D7" : "0px",
              p: 0,
              px: "12px",
              borderRadius: "0px",
              fontSize: ["11px", "15px", "18px"],
