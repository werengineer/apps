...
export const LeftSection = ({ data, count }) => {
	console.log(data);
	const [reaction, setReaction] = useState(0);
	const [reactionsCount, setReactionsCount] = useState(count);
	const [loading, setLoading] = useState(true);
	const [anchorEl, setAnchorEl] = useState(null);
	const [answersCount, setAnswerCount] = useState(data?.reactions?.length);
	...
	useEffect(() => {
		const fetchReactionAndStoy = async () => {
			setLoading(true);
			try {
	...
	const handleReaction = async ({ rID }) => {
		...
		try {
			const res = await Like({ reactionID: rID, storyID: data?._id });
			console.log(res);
		} catch (error) {
			console.log(error);
		}
